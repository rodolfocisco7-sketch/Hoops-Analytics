...
"Brooklyn Nets": {
    "Cameron Thomas":    {"id": 1134019, "pos": "SG", "alt": 191},
...
with st.sidebar:
    ...
    with st.spinner("Verificando lesiones..."):
        lesionados_df = obtener_jugadores_lesionados(equipo_sel)
    ...
    if not lesionados_df.empty:
        ...
        for _, row in lesionados_df.iterrows():
            ...
            razon_val = str(row.get('Razon', '')).lower()
            nombre_jugador = row['Jugador']
            razon_original = row.get('Razon', 'No especificado')
            
            if 'injury' in tipo_val or 'injured' in razon_val:
                st.markdown(f"🏥 **{nombre_jugador}** - Lesionado")
            elif 'suspension' in tipo_val or 'suspended' in razon_val:
                st.markdown(f"🚫 **{nombre_jugador}** - Suspendido")
            else:
                st.markdown(f"⚠️ **{nombre_jugador}** - {razon_original}")
    else:
    ...
    if contexto and contexto["hay_juego"] and contexto["rival"]:
        ...
        <div class="alert-info">
            <h4 style='margin:0 0 10px 0; color:#00D9FF;'>🆚 PRÓXIMO PARTIDO</h4>
            <p style='font-size:18px; margin:5px 0; color:#E0F4FF;'><b>{contexto['rival']}</b></p>
        ...
        st.warning("📅 No hay partidos en los próximos 7 días")
        st.session_state.rival_nombre = None
    
    st.divider()
    
    st.markdown("#### 📊 Partidos a Analizar")
    num_partidos_visualizar = st.slider("Visualizar últimos", min_value=3, max_value=7, value=7)
...
if btn_cargar:
    ...
    total_jugadores = sum(len(JUGADORES_DB[eq]) for eq in equipos_a_scrapear)
    contador = 0
    ...
    if all_data:
        ...
        st.session_state.equipos_cargados = equipos_a_scrapear
        st.balloons()
        st.rerun()
    else:
